...
MBTiles.prototype._getTileDataColumn = function(callback) {
    ...
    mbtiles._db.all('PRAGMA table_info(tiles)', function(err, rows) {
        if (err) return callback(err);
        if (!rows) return callback(new Error('Corrupted database.'));
        if (hasColumn(rows, 'tile_data1')) mbtiles.tileDataColumn = 'tile_data1';
        callback(null);
    });

    function hasColumn(tableInfo, column) {
        return tableInfo.find(function(c) { return c.name == column });
    }
};
...
MBTiles.prototype.getTile = function(z, x, y, options, callback) {
    ...
    if (options.overlay && !this.overlayColumn) return callback(new Error('no overlay column name specified'));

    // Flip Y coordinate because MBTiles files are TMS.
    if (this.tms) y = (1 << (this.wgs84 ? z - 1 : z)) - 1 - y;
    var tile_data_col = options.overlay ? this.overlayColumn : this.tileDataColumn;
    tile_data_col = (tile_data_col && tile_data_col + ' AS tile_data') || 'tile_data';
    var where = this.usekey ? 'WHERE id = ' + tileid(z, x, y) : 'WHERE zoom_level = '+z+' AND tile_column = '+x+' AND tile_row = ' + y;
