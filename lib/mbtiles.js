...
if (process.env.BRIDGE_LOG_MAX_VTILE_BYTES_COMPRESSED) {
  ...
  process.on("exit", function () {
    if (stats.count > 0) {
      fs.writeFileSync(
        os.tmpdir() + "/tilelive-bridge-stats.json",
...
function tileid(z, x, y) {
  var str = +x + +y + "";
  return "" + (str.length + 2) + (+z + 3) + str + (+y + +z);
}
...
function MBTiles(uri, callback) {
  ...
  this.tms = uri.tms;
  this.wgs84 = uri.wgs84;
  this.tileDataColumn = uri.tiledatacol;
  this.overlayColumn = uri.overlaycol;
  this.usekey = uri.usekey;
  ...
  mbtiles._db = new sqlite3.Database(mbtiles.filename, mode, function (err) {
    ...
    fs.stat(mbtiles.filename, function (err, stat) {
      ...
      if (mbtiles.tileDataColumn) {
        mbtiles.open = true;
        mbtiles.emit("open", err);
        return callback(null, mbtiles);
      }

      mbtiles._getTileDataColumn(function (err) {
        if (err) return callback(err);
        mbtiles.open = true;
        mbtiles.emit("open", err);
...
// DB integrity check.
// - @param {Function(err)} callback
MBTiles.prototype._getTileDataColumn = function (callback) {
  if (typeof callback !== "function") callback = noop;

  var mbtiles = this;
  mbtiles._db.all("PRAGMA table_info(tiles)", function (err, rows) {
    if (!err && rows) {
      if (hasColumn(rows, "tile_data1")) mbtiles.tileDataColumn = "tile_data1";
    }
    callback(null);
  });

  function hasColumn(tableInfo, column) {
    return tableInfo.find(function (c) {
...
MBTiles.prototype.getTile = function (z, x, y, options, callback) {
  ...
  if (options.overlay && !this.overlayColumn)
    return callback(new Error("no overlay column name specified"));

  // Flip Y coordinate because MBTiles files are TMS.
  if (this.tms) y = (1 << (this.wgs84 ? z - 1 : z)) - 1 - y;
  var tile_data_col = options.overlay
    ? this.overlayColumn
    : this.tileDataColumn;
  tile_data_col =
    (tile_data_col && tile_data_col + " AS tile_data") || "tile_data";
  var where = this.usekey
    ? "WHERE id = " + tileid(z, x, y)
    : "WHERE zoom_level = " +
      z +
      " AND tile_column = " +
      x +
  ...
  var sql = "SELECT " + tile_data_col + " FROM tiles " + where;
  var mbtiles = this;

  this._db.get(sql, function (err, row) {
...
MBTiles.prototype.getGrid = function (z, x, y, callback) {
  ...
  mbtiles._db.get(sqlgrid, z, x, y, function (err, row) {
    ...
    zlib.inflate(
      !Buffer.isBuffer(row.grid) ? new Buffer(row.grid, "binary") : row.grid,
      function (err, buffer) {
...
MBTiles.prototype._commit = function (callback) {
  ...
  if (mbtiles._committing)
    return mbtiles.once("commit", function () {
    ...
    Object.keys(writes).forEach(function (table) {
      switch (table) {
        ...
        default:
          ...
          while (rows.length)
            statements[table].run.apply(statements[table], rows.shift());
...
MBTiles.prototype.putTile = function (z, x, y, data, callback) {
  ...
  if (!this.open) return callback(new Error("MBTiles not yet loaded"));
  if (!this._isWritable)
    return callback(new Error("MBTiles not in write mode"));
  if (!Buffer.isBuffer(data))
    return callback(new Error("Image needs to be a Buffer"));
...
MBTiles.prototype.getGeocoderData = function (type, shard, callback) {
  return this._db.get(
    "SELECT data FROM geocoder_data WHERE type = ? AND shard = ?",
    type,
    shard,
    function (err, row) {
      if (err && err.code === "SQLITE_ERROR" && err.errno === 1)
        return callback();
      if (err) return callback(err);
...
MBTiles.prototype.putGeocoderData = function (type, shard, data, callback) {
  ...
  zlib.deflate(data, function (err, zdata) {
    ...
    source.write(
      "geocoder_data",
      type + "." + shard,
      { type: type, shard: shard, data: zdata },
      callback
    );
  });
};
...
MBTiles.prototype.geocoderDataIterator = function (type) {
      ...
      if (dataQueue[0] == doneSentinel) {
        ...
        (function (nextCb, cbValue) {
          inflate(data.row.data, function (err, buf) {
            nextCb(data.err, {
              value: { shard: data.row.shard, data: buf },
              done: false,
            });
          });
        })(nextCb, data);
  ...
  var refillBuffer = function () {
    ...
    var segmentCount = 0;
    _this._db.each(
      "SELECT shard, data FROM geocoder_data WHERE type = ? ORDER BY shard limit ?,?",
      type,
      position,
      chunkSize,
      function (err, row) {
  ...
  var maybeRefillBuffer = function () {
    if (
      dataQueue.length <= getNextIfBelow &&
      !refilling &&
      dataQueue[dataQueue.length - 1] != doneSentinel
    ) {
      refillBuffer();
    }
  };
...
MBTiles.prototype.getIndexableDocs = function (pointer, callback) {
  ...
  if (pointer.table === "carmen") {
    return this._db.all(
      "SELECT c.id AS id, c.text AS text, c.zxy, k.key_json FROM carmen c JOIN keymap k ON c.id = k.key_name LIMIT ? OFFSET ?",
      pointer.limit,
      pointer.offset,
      function (err, rows) {
        if (err) return callback(err);
  ...
  this.getInfo(
    function (err, info) {
      ...
      if (pointer.nogrids) {
        ...
        sql =
          "SELECT k.key_name AS id, k.key_json, GROUP_CONCAT(zoom_level||'/'||tile_column ||'/'||tile_row,',') AS zxy FROM keymap k JOIN grid_key g ON k.key_name = g.key_name JOIN map m ON g.grid_id = m.grid_id WHERE m.zoom_level=? GROUP BY k.key_name LIMIT ? OFFSET ?;";
...
MBTiles.prototype.geocoderMigrateDocs = function (rows, callback) {
  ...
  function tms2zxy(zxys) {
    return zxys.split(",").map(function (tms) {
      ...
      zxy[2] = (1 << (this.wgs84 ? zxy[0] - 1 : zxy[0])) - 1 - zxy[2];
      return zxy.join("/");
    });
  }
  ...
  if ("zxy" in row && text) {
    doc._id =
      parseInt(row.id, 10).toString() === row.id
        ? parseInt(row.id, 10)
        : parseInt(
            crypto.createHash("md5").update(row.id).digest("hex").substr(0, 8),
            16
          );
    doc._text = text;
    doc._zxy = row.zxy ? (source.tms ? tms2zxy(row.zxy) : row.zxy) : [];
    if (doc.score) doc._score = parseFloat(doc.score);
    if (doc.bounds)
      doc._bbox = doc.bounds.split(",").map(function (v) {
        return parseFloat(v);
      });
    delete doc.score;
    delete doc.bounds;
...
MBTiles.prototype.geocoderCentroid = function (id, zxy, callback) {
  ...
  this.getGrid(mid[0], mid[1], mid[2], function (err, grid) {
    ...
    callback(
      null,
      sm.ll([256 * mid[1] + cxy[0] * 4, 256 * mid[2] + cxy[1] * 4], mid[0])
    );
  });
};
