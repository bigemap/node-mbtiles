...
MBTiles.prototype._getTileDataColumn = function(callback) {
    ...
    mbtiles._db.all('PRAGMA table_info(tiles)', function(err, rows) {
        ...
        if (hasColumn(rows, 'tile_data1')) mbtiles._tile_data_col = 'tile_data1';
        callback(null);
    });
...
MBTiles.prototype.getTile = function(z, x, y, callback) {
    ...
    var tileDataCol = (this._tile_data_col &&  this._tile_data_col + ' as tile_data') || 'tile_data';
    var sql = 'SELECT ' + tileDataCol + ' FROM tiles WHERE zoom_level = ? AND tile_column = ? AND tile_row = ?';
    var mbtiles = this;
